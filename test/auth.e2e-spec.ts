...
describe('AuthController (e2e)', () => {
  ...
  it('should log an employee in', () => {
    return request(app.getHttpServer())
      ...
      .expect(response => {
        ...
        expect(userMock).toEqual(expect.objectContaining(response.body.user));
      });
  });
  ...
  it('should not log an employee in if password is incorrect', () => {
    return request(app.getHttpServer())
      .post('/api/auth/signin')
      .send({ email: userMock.email, password: faker.internet.password() })
      .accept('application/json')
      .expect(401)
      .expect(response => {
  ...
  it('should not log an employee in if email is not a valid email address', () => {
    return request(app.getHttpServer())
      .post('/api/auth/signin')
      .send({ email: 'invalid@test', password: userMock.password })
      .accept('application/json')
      .expect(400)
      .expect(response => {
